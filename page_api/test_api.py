...
def cart_product_id(product_id):
    """ Фикстура сначала добавляет товар в корзину, затем находит его внутренний ID в корзине по внешнему ID. Используется во многих последующих тестах. """
    # добавляем товар
...
def test_add_product(product_id):
    """ Тест проверяет успешное добавление товара в корзину. Сначала убеждается, что товара ещё нет в корзине, затем добавляет его и снова проверяет наличие. """
...
def test_delete_product(product_id, cart_product_id):
    """ Тест проверяет возможность успешного удаления товара из корзины. Сначала убеждается, что товар присутствует в корзине, затем удаляет его и проверяет отсутствие. """
...
def test_restore_product(product_id, cart_product_id):
    """ Тест проверяет успешность восстановления удаленного товара в корзину. Сначала удаляется товар, затем восстанавливается и проверяется его присутствие. """
    with allure.step("Удаляем товар из корзины"):
...
def test_get_cart():
    """ Тест проверяет успешное получение содержимого корзины. Убедимся, что запрос выполнен успешно и данные возвращаются в виде массива продуктов. """
...
def test_clear_cart(cart_product_id):
    """ Тест проверяет успешность полной очистки корзины. Сначала убеждаемся, что в корзине есть хотя бы один товар, затем очищаем её и проверяем пустоту. """
