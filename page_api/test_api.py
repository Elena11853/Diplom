...
def test_add_product(product_id):
    """ Тест проверяет успешное добавление товара 
    в корзину. Сначала убеждается, 
    что товара ещё нет в корзине, 
...
@allure.description("Проверка возможности удалить " \
"товар из корзины.")
def test_delete_product(product_id, cart_product_id):
    """ Тест проверяет возможность успешного удаления 
    товара из корзины. 
    Сначала убеждается, что товар присутствует в корзине, 
...
@allure.description("Проверка возможности восстановить " \
"удаленный товар в корзину.")
def test_restore_product(product_id, cart_product_id):
    """ Тест проверяет успешность восстановления 
    удаленного товара в корзину. 
    Сначала удаляется товар, 
...
@allure.description("Проверка правильности отображения " \
"содержимого корзины.")
def test_get_cart():
    """ Тест проверяет успешное получение содержимого корзины. 
    Убедимся, что запрос выполнен успешно и данные возвращаются 
    в виде массива продуктов. """
...
@allure.description("Проверка полного удаления всех " \
"товаров из корзины.")
